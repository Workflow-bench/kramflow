...
2. Run every file in `supabase/migrations/`, in filename order, in the project's SQL Editor (or `supabase db push`). The first file, `20260909091731_schema_base.sql`, is the base schema (`sessions`, `programs`, `live_state`, `activity_log`, etc., added to the Realtime publication); the rest bring it up to what the app expects: `events`/`event_collaborators`, event-scoped RLS on every table, the RPCs the code calls, rate limiting, display profiles, share links with TV codes, and so on. `supabase/schema.sql` is an older single-file snapshot and is no longer needed.
3. Files are named by their Supabase version timestamp so the Supabase GitHub integration can match them to the production migration history. **Verified 2026-09-20:** applying all 17 files in this order to an empty Postgres reproduces the production schema exactly (columns, constraints, indexes, RLS policies, functions, triggers, Realtime tables). It does not need `schema.sql`, and running `schema.sql` first gives the same result. Two things exist only in production and are intentionally not recreated: the stale pre-multitenant 3-argument `replace_session_programs` overload, and the platform-owned `rls_auto_enable` function.
